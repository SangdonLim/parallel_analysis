# Figure 4
# Percent correct of the selected PA procedures for the categorical variable
# ------------------------------------------------------------------------------
...
cell_values[, 1] <- c(0, 1, 0, 1, 0, 1, 0, 1) # factor correlation
cell_values[, 2] <- c(1, 1, 1, 1, 0, 0, 0, 0) # factor loading
cell_values[, 3] <- c(4, 4, 2, 2, 4, 4, 2, 2) # number of categories
...
for (img_type in c("eps", "emf", "svg")) {
  ...
  setEPS()
  if (img_type == "eps") {
    postscript("figures/Figure_4.eps", width = 6.8, height = 4.5)
  }
  if (img_type == "emf") {
    emf(
      "figures/Figure_4.emf", width = 6.8, height = 4.5,
      family = "Arial", coordDPI = 2540, emfPlus = FALSE
    )
  }
  if (img_type == "svg") {
    svg(
      "figures/Figure_4.svg", width = 6.8, height = 4.5
    )
  }
  ...
  for (cell_idx in 1:8) {
    ...
    tmp <- subset(
      ...
      arg_data[["n_cats"]]   == cell_values[cell_idx, 3]
    )
